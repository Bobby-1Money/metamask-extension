 ...
 *
 * @param data - Weekly digest data (FP rate, promotions, regressions, graduation).
 */
export function formatWeeklyDigest(data: WeeklyDigestData): {
  ...
  const promotedText =
    data.metricsPromoted.length > 0
      ? data.metricsPromoted
          .map((m) => `${m.name}: ${m.fromTier} -> ${m.toTier}`)
          .join(', ')
      : 'none';

  const skipText =
    data.skipLabelUses.length > 0
      ? data.skipLabelUses.map((s) => `#${s.prNumber} — ${s.reason}`).join(', ')
      : 'none';

  const headerBlock = richTextSection([
    emojiEl('bar_chart'),
    textEl(' Performance Quality Gate — Weekly Digest', { bold: true }),
  ]);

  const summaryElements: Record<string, unknown>[] = [
    textEl('False positive rate: ', { bold: true }),
    textEl(
      `${(data.falsePositiveRate * 100).toFixed(1)}% (target: <${(data.falsePositiveTarget * 100).toFixed(0)}%)\n`,
    ),
    textEl('Metric graduation: ', { bold: true }),
    textEl(`${data.metricsPromoted.length} promoted (${promotedText})\n`),
    textEl('Total enforced: ', { bold: true }),
    textEl(`${data.totalEnforced}/${data.totalMetrics}\n\n`),
    textEl('Regressions this week: ', { bold: true }),
    textEl(
      `${data.regressionsThisWeek.total} (${data.regressionsThisWeek.real} real, ${data.regressionsThisWeek.flake} flake)\n`,
    ),
    textEl('Skip label uses: ', { bold: true }),
    textEl(`${skipText}\n`),
  ];
  ...
  const blocks: SlackBlock[] = [
    headerBlock,
    divider(),
    richTextSection(summaryElements),
  ];
